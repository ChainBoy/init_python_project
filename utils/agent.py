...
SUBPLATFORM = {
    ...
    'linux': (
        ('amd64', 'Linux amd64'), # 32bit
        ('i586', 'Linux i586'), # 32bit
        ('i486', 'Linux i486'), # 32bit
        ('i386', 'Linux i386'), # 32bit
...
def generate_navigator(platform=None, navigator=None):
    ...
    if type(platform) in (str, unicode):
        platform_choices = [platform]
    elif type(platform) in (list, tuple):
    ...
    if type(navigator)in (str, unicode):
        navigator_choices = [navigator]
    elif type(navigator) in (list, tuple):
...
def new():
    return generate_navigator()['user_agent']
